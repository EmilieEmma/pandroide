...
def compute_angle_distance(img,cont_tag,tag_size):
    _,img_w = img.shape
    ...
    """
    print "dist min =",tools.DIST_MIN
    print "height min = ",tools.HEIGHT_MIN
    print "dist max =",tools.DIST_MAX
    print "height max = ",tools.HEIGHT_MAX
    print 'tag height = ', tag_h
    print 'tag width = ', tag_w
    """
    # distance ("produit en croix")
    dist = dist_h = tools.DIST_MIN*tools.HEIGHT_MIN/tag_h # theoreme 3D (DDD)
    ...
    # TODO : distance du tag d'apres la hauteur, la largeur ou le perimetre ou une moyenne ou autre?
    if dist == 0 :
        return (0,0)
    # angle <- trigonometry (sinus : opposite/hypothenusis)
    # the distance from center line of the image to the tag centroid
    pix_midd = fabs(tagx-midd_w) # opposite
    dist_midd = pix_midd/tools.CM_PIX
    print "MIDDLE =",dist_midd
    print "ARCSIN_4 = ",asin(0.0001)
    print "div = ",(dist_midd/float(dist))
...
def obtain_tag_info(img, tag, contours, hierarchy, tag_cont):
    ...
    """
    print "direct = ",cont_dir
    print "separation children :",part1-sep
    print "get part id :",read1-part1
    print "get part dir :",read2-part2
    """
    if tools.DIST_ANGLE:
        minxy, maxxy = contour_min_max(tag_cont)
        print "contour tag ",tag_cont
        print "min =",minxy," - max =",maxxy
        tag_size = (maxxy[1]-minxy[1],maxxy[0]-minxy[0])
        # retrieving the distance and the angle (in radian) from me
        dist, angle = compute_angle_distance(img, tag_cont,tag_size)
        print "found distance = ",dist,"\nfound angle = ",angle
        return id_bot,direct, dist, angle
    else:
        return id_bot,direct
...
def is_real_tag(tag):
    ...
    isBlackWhite = lambda x : x>=255/3 and x<=5*255/6
    for fact in range(3):
        moys = []
        # for ligns
        ...
        if fact!=1:
            ...
            nb_edges_ok += map(isBlackWhite,moys).count(True)
        print "nb = ",nb_edges_ok
    # returning whether the image really is a tag
    if (True if nb_edges_ok>=4.5 else False):
...
def lecture_tag(img,tag_found, tag_cont):
    ...
    dt = time.time()
    #thresh = tools.thresholding(tag_found)
    ...
    gt = time.time()
    thresh = tools.apply_filters(tag)
    #print "temps thresh apply filter : ",time.time()-gt
    #print 'temps canny apply filter : ',gt-dt
    # thresh = cv2.erode(thresh, None, iterations = 1) # usefull?
    # extract hierarchy of the tag
    #cv2.imshow("infos canny", canny)
    #cv2.imshow("infos thresh", thresh)

    contours, hierarchy = cv2.findContours(thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
    #tr = img.copy()
    #cv2.drawContours(tr,[tag_cont],-1,(0,255,0),3)
    #cv2.imshow("infos found canny",tr)
    #cv2.imshow("infos found canny threshed",tag)
    #cv2.waitKey(0)
    #print 'tag contours 0 ',tag_cont
    res = obtain_tag_info(img, tag, contours, hierarchy,tag_cont)
    ft = time.time()
    #print "Temps lecture :",ft-dt
    return res
