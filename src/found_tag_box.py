...
"""
def found_tag_img(img, demo=False):
    #try:
    ...
    if demo:
        ...
        view_demo(img,contours)
    if contours:
        dt = time.time()
        # getting a simple image of the tag (only)
        # checking the tag
        robots_view = tools.check_tags(img,gray,contours,demo=demo)
...
def getTagBox_vEmilias(gray):
    ...
    for i,cnt in enumerate(contours):
        ...
        if not tools.isPerimeterOK(cv2.arcLength(approx,True)) :#or not tools.isAreaOK(cv2.contourArea(approx)):
            continue
        # filter contour by almost one child
        # no parent?

        if tools.verify_hierarchy(hierarchie[0], i):
            """
            gray1 = gray.copy()
            print "HIERARCHY = ",hierarchie[0,i]
            cv2.drawContours(gray1, [cnt], -1, (0,255,0), 3)
